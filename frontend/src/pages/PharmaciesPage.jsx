...
function PharmaciesPage() {
  ...
  const fetchPharmacies = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/pharmacies/list", {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/api/pharmacies/add",
        form,
        {
          headers: {
            Authorization: `Bearer ${userToken}`,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/pharmacies/delete/${id}`, {
        headers: {
          Authorization: `Bearer ${userToken}`,
        },
      });
