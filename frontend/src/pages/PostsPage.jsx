...
function PostsPage() {
  ...
  async function fetchPostMessage() {
    try {
      const res = await axios.get("http://localhost:5000/api/posts/get-post-message", {
        headers: { Authorization: `Bearer ${userToken}` }
      });
  ...
  const handleSaveMessage = async () => {
    ...
    try {
      ...
      await axios.post(
        "http://localhost:5000/api/posts/update-post-message",
  ...
  const forcePostNow = async (postId) => {
    try {
      await axios.post(
        `http://localhost:5000/api/posts/force-post/${postId}`,
        {},
        { headers: { Authorization: `Bearer ${userToken}` } }
      );
      fetchScheduledPosts();
  ...
  const cancelScheduledPost = async (postId) => {
    ...
    try {
      await axios.delete(
        `http://localhost:5000/api/posts/cancel/${postId}`,
        { headers: { Authorization: `Bearer ${userToken}` }  }
      );
      fetchScheduledPosts();
  ...
  const deleteAllPosts = async () => {
    ...
    try {
      await axios.delete(
        "http://localhost:5000/api/posts/delete-all",
        { headers: { Authorization: `Bearer ${userToken}` }}
      );
      fetchScheduledPosts();
